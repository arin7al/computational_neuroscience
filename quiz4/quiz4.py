...
def meanSpikeAndVarianceCount(neuron_name, data):
...
with open('tuning_2.7.pickle', 'rb') as f:
    ...
    plot2 = plt.figure(2)
    (mean, variance) = meanSpikeAndVarianceCount('neuron1',data)
    plt.plot(mean, variance, 'bo', label='Neuron1')
    (mean, variance) = meanSpikeAndVarianceCount('neuron2',data)
    plt.plot(mean, variance, 'ro', label='Neuron2')
    (mean, variance) = meanSpikeAndVarianceCount('neuron3',data)
    plt.plot(mean, variance, 'go', label='Neuron3')
    (mean, variance) = meanSpikeAndVarianceCount('neuron4',data)
